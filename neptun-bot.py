...
# guild_id : {user_id: [count, [(author_id, raison, preuve)]]}
bot.warnings = {}
...
async def check_is_config_on_ready(guild: discord.Guild):
    ...
    try:
        ...
        print(
            f"{Fore.RED}{guild.id} ✘ Channel_welcome n'a pas été configuré{Fore.RESET}")
        logging.warning(f'{guild.id} Channel_welcome is not config')
        channel_welcome_is_config = False
    try:
        data['invite_link']
            ...
            for line in lines:
                ...
                try:
                    ...
                    bot.warnings[guild.id][member_id][1].append(
                        (admin_id, sanction_id, reason))

                except KeyError:
                    bot.warnings[guild.id][member_id] = [
                        1, [(admin_id, sanction_id, reason)]]
...
async def on_member_join(member: discord.Member):
    ...
    embed = discord.Embed(
        title="Bienvenue", description=f"{member.mention}, bienvenue sur le serveur **{guild.name}** !", color=color)
...
async def clear(ctx, *, nombre: int = None):
    ...
    if await check_is_config(guild=ctx.guild) is False:
        ...
        logging.warning(
            f"{ctx.author} a utilisé une commande mais le bot n'est pas configuré")
    ...
    embed = discord.Embed(
        title=f"Le channel {ctx.channel} a été clear !", color=color)
    embed.set_thumbnail(url=ctx.author.avatar_url)
    embed.add_field(name="Modérateur", value=ctx.author.mention, inline=True)
    embed.add_field(name="Nombre de messages supprimés",
                    value=nombre, inline=False)
    embed.set_footer(text=f"Date • {datetime.datetime.now()}")
    ...
    logging.info(
        f"{ctx.author} a clear {nombre} messages dans le channel {ctx.channel}")
...
async def ban(ctx, user: discord.User, *, raison="Aucune raison fournie"):
    ...
    try:
        ...
        bot.warnings[ctx.guild.id][user.id][1].append(
            (ctx.author.id, 1, raison))
    except KeyError:
...
async def kick(ctx, user: discord.User, *, raison="Aucune raison fournie"):
    ...
    embed_user.add_field(
        name="Modérateur", value=ctx.author.mention, inline=True)
    embed_user.add_field(
        name="Discord", value=data['invite_link'], inline=True)
    try:
        await user.send(embed=embed_user)
    except AttributeError:
        pass
...
async def unban(ctx, user, *, raison="Aucune raison fournie"):
    ...
    try:
        ...
        logging.warning(
            f"{ctx.author} a utilisé la commande '/unban {user}', ValueError: {value}")
    ...
    async def is_banned():
        for ban_entry in banned_users:
            ...
            if (user.name, user.discriminator) == (user_name, user_discriminator):
                ...
                logging.info(
                    f"{ctx.author} a dé-banni {user}, raison : {raison}")
                return True
    if await is_banned() != True:
        await ctx.send(embed=discord.Embed(description=f":warning: Aucun membre banni ne correspond à : **{user_base}** :negative_squared_cross_mark: Faites **/ban_list** pour voir la liste des membres bannis.", color=get_color(0xf54531, 0xf57231, 0xf53145)), hidden=True)
        logging.warning(
            f"{ctx.author} a utilisé la commande '/unban {user_base}', Aucun membre correspondant à : {user_base}")
...
async def ban_list(ctx):
    ...
    if len(banned_users_list) == 0:
        embed = discord.Embed(
            title="Aucun membre n'est banni sur le serveur", color=color)
    else:
        embed = discord.Embed(
            title="Voici la liste des membres bannis du discord", color=color)
...
async def tempban(ctx, user: discord.User, duration: int, time: str, *, raison="Aucune raison fournie"):
    ...
    if "s" == time:
        ...
        embed.add_field(name="Utilisateur banni",
                        value=user.mention, inline=True)
        embed.add_field(name="Raison", value=raison, inline=True)
        embed.add_field(
            name="Durée", value=f"{duration} seconde(s)", inline=True)
        ...
        embed_user = discord.Embed(title="Vous avez été banni temporairement !",
                                   ...
                                   color=color)
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de banissement",
                             value=f"{duration} seconde(s)", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
        ...
        try:
            ...
            return
        logging.info(
            f"{ctx.author} a banni temporairement {user} : {duration} {time}, raison : {raison}")
        ...
        embed.add_field(name="Raison", value=raison, inline=True)
        embed.add_field(
            name="Durée", value=f"{duration} minute(s)", inline=True)
        embed.add_field(name="Modérateur",
                        value=ctx.author.mention, inline=True)
        embed.set_footer(text=f"Date • {datetime.datetime.now()}")
        embed_user = discord.Embed(title="Vous avez été banni temporairement !",
        ...
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de banissement",
                             value=f"{duration} minute(s)", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
        ...
        embed.add_field(name="Raison", value=raison, inline=True)
        embed.add_field(
            name="Durée", value=f"{duration} heure(s)", inline=True)
        embed.add_field(name="Modérateur",
                        value=ctx.author.mention, inline=True)
        embed.set_footer(text=f"Date • {datetime.datetime.now()}")
        embed_user = discord.Embed(title="Vous avez été banni temporairement !",
        ...
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de banissement",
                             value=f"{duration} heure(s)", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
        ...
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de banissement",
                             value=f"{duration} jour(s)", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
        ...
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de banissement",
                             value=f"{duration} mois", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
...
async def tempmute(ctx, user: discord.User, duration: int, time: str, *, raison="Aucune raison fournie"):
    ...
    if "s" == time:
        embed = discord.Embed(title=f"{user.name} a été **mute temporairement** !",
                              description=f"Cet utilisateur n'a pas respecté les règles du serveur !", color=color)
        embed.add_field(name="Utilisateur mute",
                        value=user.mention, inline=True)
        embed.add_field(name="Raison", value=raison, inline=True)
        embed.add_field(
            name="Durée", value=f"{duration} seconde(s)", inline=True)
        ...
        embed_user = discord.Embed(title="Vous avez été mute temporairement !",
                                   ...
                                   color=color)
        embed_user.add_field(name="Raison", value=raison, inline=True)
        embed_user.add_field(name="Temps de mute",
                             value=f"{duration} seconde(s)", inline=True)
        embed_user.add_field(
            name="Modérateur", value=ctx.author.mention, inline=True)
        embed_user.set_footer(text=f"Date • {datetime.datetime.now()}")
        ...
        logging.info(
            f"{ctx.author} a mute temporairement {user} : {duration} {time}, raison : {raison}")
    elif "m" == time:
        duration_min = duration * 60
        embed = discord.Embed(title=f"{user.name} a été **mute temporairement** !",
                              description=f"Cet utilisateur n'a pas respecté les règles du serveur !", color=color)
...
async def unmute(ctx, user: discord.User, *, raison="Aucune raison fournie"):
    ...
    if role_mute in user.roles:
        ...
        await ctx.send(embed=discord.Embed(description=f":warning: **{user}** n'est pas mute :negative_squared_cross_mark:", color=get_color(0xf54531, 0xf57231, 0xf53145)), hidden=True)
        logging.warning(
            f"{ctx.author} a utilisé la commande '/unmute {user}', ce membre n'est pas mute")
...
async def report(ctx, user: discord.User, raison, *, preuve="Aucune preuve fournie"):
    ...
    await channel_logs.send(embed=embed)
    logging.info(
        f"{ctx.author} a report {user}, raison : {raison}, preuve : {preuve}")
...
async def sanctions(ctx, user: discord.User):
    ...
    embed_title = discord.Embed(
        title="Sanctions", description=f"Listes des sanctions de **{user}**", colour=color)
    try:
        try:
            await sanctions_files()
        except:
        ...
        for author_id, sanction_id, raison in bot.warnings[ctx.guild.id][user.id][1]:
            ...
            embed = discord.Embed(
                title=f"{i}. {sanction_name}", description=":warning:", color=color)
            embed.add_field(name="Raison", value=raison)
            embed.add_field(name="Modérateur", value=author.mention)
            await ctx.send(embed=embed, hidden=True)
            i += 1
    except KeyError:  # no warnings
        embed_title.add_field(name=":white_check_mark:",
                              value="Ce membre n'a aucune sanction !")
        embed_title.set_footer(text=user, icon_url=user.avatar_url)
...
async def user_info(ctx, user: discord.Member):
    await ctx.defer(hidden=True)
    guild: discord.Guild = ctx.guild
    embed = discord.Embed(title="User informations",
                          description=f"**Informations** sur {user.mention}", color=get_color(0x42c5f5, 0xf54275, 0x5bfc58))
    embed.add_field(name="Pseudo complet", value=user, inline=True)
    embed.add_field(name="ID", value=user.id, inline=True)
    embed.add_field(name="Roles sur le serveur",
                    value=len(user.roles) - 1, inline=False)
    embed.add_field(name="Role le plus haut",
...
async def server_info(ctx: discord.ext.commands.context.Context):
    ...
    embed = discord.Embed(title=f"Informations du serveur\n{guild.name}", description=guild_description, color=get_color(
        0xedda5f, 0xedab5f, 0xbb76f5))
    ...
    embed.add_field(name="Nombre de salons",
                    value=len(guild.channels), inline=True)
    embed.add_field(name="Nombre de salons vocaux",
                    value=len(guild.voice_channels), inline=False)
    embed.add_field(name="Nombre de salons textuels",
                    value=len(guild.text_channels), inline=True)
    embed.add_field(name="Création du serveur",
...
async def config_server(ctx):
    ...
    welcome_config: discord.Message = await channel.send(embed=discord.Embed(
        ...
        components=[[
            Button(style=ButtonStyle.green, label="Oui",
                   custom_id="yes_welcome_channel"),
            Button(style=ButtonStyle.red, label="Non",
                   custom_id="no_welcome_channel")
        ]])
...
async def help(ctx, command):
    ...
    if command == "all_commands":
        ...
        embed = discord.Embed(title="Liste de toutes les commandes les commandes",
                              color=color)
        embed.add_field(
            name="``/clear``", value="Cette commande permet d'effacer un certains nombre de message, pour plus de renseignement faites **/help clear**", inline=False)
        embed.add_field(
            name="``/kick``", value="Cette commande permet d'expulser un membre du discord, pour plus de renseignement faites **/help kick**", inline=False)
        embed.add_field(
            name="``/ban``", value="Cette commande permet de bannir un membre du discord, pour plus de renseignement faites **/help ban**", inline=False)
        embed.add_field(
            name="``/unban``", value="Cette commande permet de dé-bannir un membre du discord, pour plus de renseignement faites **/help unban**", inline=False)
        embed.add_field(
            name="``/tempban``", value="Cette commande permet de bannir temporairement un membre du discord, pour plus de renseignement faites **/help tempban**", inline=False)
        ...
        embed.add_field(
            name="``/unmute``", value="Cette commande permet dé-mute un membre du discord, pour plus de renseignement faites **/help unmute**", inline=False)
        embed.add_field(
            name="``/report``", value="Cette commande permet de report un membre du discord pour plus de renseignement faites **/help report**", inline=False)
        ...
        embed.add_field(
            name="``/warn``", value="Cette commande permet d'avertir un membre du discord, pour plus de renseignement faites **/help ban_list**", inline=False)
        ...
        embed.add_field(name="``/blacklist_add``",
                        value="Cette commande permet des membres à la blacklist, pour plus de renseignement faites **/help blacklist_add**", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        embed.add_field(name="``/documentation``",
                        value="Cette commande permet d'obtenir un lien vers ma documentation, pour plus de renseignement faites **/help documentation**", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "clear":
        author = ctx.author
        embed = discord.Embed(title="Commande clear", description="***/clear***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'effacer un certains nombre de message", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/clear [nombre de message]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "kick":
        author = ctx.author
        embed = discord.Embed(title="Commande kick", description="***/kick***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'expulser un membre du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/kick [membre] [*raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "ban":
        author = ctx.author
        embed = discord.Embed(title="Commande ban", description="***/ban***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de bannir un membre du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/ban [membre] [*raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "unban":
        author = ctx.author
        embed = discord.Embed(title="Commande unban", description="***/unban***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de dé-bannir un membre du discord", inline=False)
        embed.add_field(
            name="Utilisation", value="``/unban [membre] [*raison]`` :warning: l'option **membre** doit être rempli sous cette forme ***user#1234***", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande tempban", description="***/tempban***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de bannir temporairement un membre du discord", inline=False)
        embed.add_field(
            name="Utilisation", value="``/tempban [membre] [durée : nombre] [temps : seconde / minute / heure / jour / mois] [*raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande tempmute", description="***/tempmute***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de mute temporairement un membre du discord", inline=False)
        embed.add_field(
            name="Utilisation", value="``/tempmute [membre] [durée : nombre] [temps : seconde / minute / heure / jour / mois] [*raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "unmute":
        author = ctx.author
        embed = discord.Embed(title="Commande tempban", description="***/tempmute***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet dé-mute un membre du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/unmute [membre] [*raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "report":
        author = ctx.author
        embed = discord.Embed(title="Commande report", description="***/report***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de report un membre du discord", inline=False)
        embed.add_field(
            name="Utilisation", value="``/report [membre] [raison] [*preuve: :warning: url vers une image :warning:]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande ban_list", description="***/ban_list***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'obtenir la liste des membres bannis du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/ban_list``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande sanctions", description="***/sanctions***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'obtenir la liste des sanctions d'un membre du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/sanctions [membre]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "warn":
        author = ctx.author
        embed = discord.Embed(title="Commande warn", description="***/warn***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande d'avertir un membre du discord", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/warn [membre] [raison]``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande serveur information", description="***/server_info***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'obtenir des informations sur le serveur", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/server_info``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Commande user information", description="***/user_info***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'obtenir des informations à propos d'un membre du serveur", inline=False)
        embed.add_field(name="Utilisation",
        ...
        embed = discord.Embed(title="Commande configuration", description="***/config_server***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet de configurer le bot pour le serveur", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/config_server``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "blacklist_add":
        author = ctx.author
        embed = discord.Embed(title="Commande ajout à la blacklist", description="***/blacklist_add***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'ajouter des membres à la blacklist", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/blacklist_add``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    elif command == "documentation":
        author = ctx.author
        embed = discord.Embed(title="Commande documentation", description="***/blacklist_add***",
                              color=color)
        embed.add_field(name="A quoi sert cette commande ?",
                        value="Cette commande permet d'obtenir un lien vers ma documentation", inline=False)
        embed.add_field(name="Utilisation",
                        value="``/documentation``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
...
async def on_button_click(interaction: Interaction):
    ...
    if message_custom_id == "yes_welcome_channel":
        ...
        async def get_channel_id(First_time=True):
            try:
                ...
                await message.channel.send(embed=discord.Embed(
                    ...
                    components=[[
                        Button(style=ButtonStyle.green, label="Oui",
                               custom_id="yes_logs_channel"),
                        Button(style=ButtonStyle.red, label="Non",
                               custom_id="no_logs_channel")
                    ]],
                    type=7)
        ...
        async def get_channel_id(First_time: bool):
            ...
            try:
                ...
                embed = discord.Embed(
                    ...
                    color=get_color(0x3ef76f, 0xe8f73e, 0xf73e3e))
                embed.add_field(name="Channel de bienvenue",
                                value=f"<#{data['channel_welcome']}>", inline=False)
                embed.add_field(name="Channel de logs",
                                value=f"<#{data['channel_logs']}>", inline=False)
                embed.set_footer(text=f"Date • {datetime.datetime.now()}")
        ...
        embed.add_field(name="Channel de logs",
                        value=f"<#{data['channel_logs']}>", inline=False)
        embed.add_field(name="Administateur",
                        value=interaction.author.mention, inline=False)
        embed.set_footer(text=f"Date • {datetime.datetime.now()}")
        ...
        embed = discord.Embed(title=f"{interaction.author} a terminé la configuration du bot", color=get_color(
            0x3ef76f, 0xe8f73e, 0xf73e3e))
        embed.add_field(name="Channel de bienvenue",
                        value=f"<#{data['channel_welcome']}>", inline=False)
        embed.add_field(name="Channel de logs",
                        value=f"<#{data['channel_logs']}>", inline=False)
...
async def on_message_delete(message: discord.Message):
    if message.author != bot.user:
        ...
        with open(f'guilds/{guild.id}/config.json') as infile:
            data = json.load(infile)
        try:
            channel_logs: discord.TextChannel = bot.get_channel(
                data['channel_logs'])
        except KeyError:
            pass
        embed = discord.Embed(
        ...
        try:
            ...
            pass
        logging.info(
            f"{message.author} a supprimé le message {message.id}, contenue : '{message.content}'")
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    if before.author != bot.user:
        ...
        try:
            ...
            pass
        logging.info(
            f"{before.author} a edité le message {before.id}, avant : '{before.content}', après : '{after.content}'")
...
async def on_message(message):
    ...
    hey_respond_list = ['Bonjour', 'Bonsoir',
                        'Salut', 'Heyyy', 'Hello', 'Coucou']
...
async def on_slash_command_error(ctx, error: discord.errors):
    if isinstance(error, commands.MissingRequiredArgument):
        ...
        embed = discord.Embed(title="Erreur", description=error, color=get_color(
            0xf54531, 0xf57231, 0xf53145))
        embed.add_field(name="Permissions requises",
                        value=f"**{error.missing_perms[0]}**")
        await ctx.send(embed=embed, hidden=True)
        logging.warning(f"{ctx.author} a obtenu une erreur : {error}")
    elif isinstance(error, commands.BotMissingPermissions):
        embed = discord.Embed(title="Erreur", description=error, color=get_color(
            0xf54531, 0xf57231, 0xf53145))
        embed.add_field(name="Permission(s) requise(s)",
                        value=f"**{error.missing_perms[0]}**")
        await ctx.send(embed=embed, hidden=True)
        ...
        embed = discord.Embed(title="Erreur", description="L'user spécifié est introuvable", color=get_color(
            0xf54531, 0xf57231, 0xf53145))
        await ctx.send(embed=embed, hidden=True)
        logging.warning(f"{ctx.author} a obtenu l'erreur : {error}")
