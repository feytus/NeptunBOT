...
@slash.slash(name="serverinfo", description="Permet d'obtenir des informations sur le serveur")
@has_permissions(send_messages=True, read_messages=True)
async def serverinfo(ctx):
    await ctx.defer(hidden=True)
    server: discord.Guild = ctx.guild
    embed=discord.Embed(title=f"Informations sur le serveur\n**{server.name}**", description=server.description, color=get_color(0x32a854, 0x32a4a8, 0xa83232))
    embed.add_field(name="Serveur id", value=server.id, inline=True)
    embed.add_field(name="Fondateur", value=server.owner, inline=True)
    embed.add_field(name="Nombre de membre", value=server.member_count, inline=False)
    embed.add_field(name="Nombre de salons", value=len(server.channels), inline=False)
    embed.add_field(name="Nombre de salons vocaux", value=len(server.voice_channels), inline=False)
    embed.add_field(name="Nombre de salons textuels", value=len(server.text_channels), inline=True)
    embed.add_field(name="Date de creation du serveur", value=server.created_at, inline=False)
    embed.set_thumbnail(url=server.icon_url)
    await ctx.send(embed=embed, hidden=True)

@slash.slash(name="config", description="Permet de configurer le bot pour l'adapter au serveur discord")
@has_permissions(send_messages=True, read_messages=True)
async def serverinfo(ctx):
    await ctx.defer(hidden=True)
    ctx.send(embed=discord.Embed(title="Config", description="Avez-vous déjà un channel logs ?"))
...
async def help(ctx, command):
    ...
    if command == "all_commands":
        ...
        , value="Cette commande permet d'avertir un membre du discord, pour plus de renseignement faites **/help warn**", inline=False)
        embed.add_field(name="``/serverinfo``"
        , value="Cette commande permet d'obtenir des informations sur le discord, pour plus de renseignement faites **/help serverinfo**", inline=False)
        ...
        color=color)
        embed.add_field(name="A quoi sert cette commande ?", value="Cette commande permet d'avertir un membre du discord", inline=False)
        ...
        author = ctx.author
        embed= discord.Embed(title="Commande serverinfo", description="***/warn***",
        color=color)
        embed.add_field(name="A quoi sert cette commande ?", value="Cette commande permet d'obtenir des informations sur le discord", inline=False)
        embed.add_field(name="Utilisation", value="``/serverinfo``", inline=False)
        embed.set_footer(text=author, icon_url=author.avatar_url)
        await ctx.send(embed=embed, hidden=True)
    logging.info(f"{ctx.author} a utilisé la commande /help {command}")
